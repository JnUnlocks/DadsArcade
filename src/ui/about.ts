...
const STORY: ReadonlyArray<string> = [
  ...
  "The old rules are all still in here, because they were good rules. Only two shots on screen at once. Ducks that flush in an arc. Jellyfish that each hunt you differently instead of moving as one clump. A frog that can only move one hop at a time, and has to live with where that hop put it. Four rows at once worth more than four rows one at a time.",
